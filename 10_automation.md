...
В архиве с тестовым примером (см. раздел [Получение образа и тестовых примеров](3_quickstart.md#test_sample)--ССЫЛКА--) находится скрипт `automation.sh`,
...
* Записывает тестовый сценарий, используя скрипт `run_record_sampleX.exp`. Запись сценария вручную описана в разделе [Запись сценария работы](3_quickstart.md#record_scenario) --ССЫЛКА--
* Выполняет воспроизведение записанного сценария. Воспроизведение сценария детально описано в разделе [Воспроизведение сценария и сбор данных для анализа](3_quickstart.md#replay_scenario) --ССЫЛКА--.
