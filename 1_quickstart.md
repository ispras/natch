...
**Решение**: проверьте, используете ли вы опцию `enable-kvm` при запуске QEMU.
...
**Решение**: процесс обработки архива продолжается в фоне. При его завершении в консоли появится надпись `Processing ___ is done!`

-----------------------------------------------------------------------------------

**Проблема**: Flame graph долго работает/завис.
...
Чтобы прервать процесс построения графа нужно остановить весь *SNatch* (запустить скрипт `snatch_stop.sh`). В последующих релизах механиз будет доработан.
