...
## 1.2. Подготовка виртуализованной среды в формате QEMU
...
Примерный алгоритм проброса виртуализации для трехуровневого стенда: Windows 11+AMD Процессор (хостовая ОС  рабочей станции) ->VirtualBox (хостовая ОС рабочей станции)-> ubuntu+kvm+qemu (хостовая ОС Natch) ->lubuntu (гостевая ОС) приведён ниже. Перед установкой KVM в гостевой ОС необходимо выполнить следующие настройки среды виртуализации VirtualBox в хостовой ОС Natch (для нормально функционирования системы, параметры должны быть в два раза больше чем на скринах, видеопамяти, по возможности вашей системы, кратно больше):

![image](https://user-images.githubusercontent.com/46653985/194891613-2b99d4b2-00d0-441e-b4db-6644c9d2bd89.png)

![image](https://user-images.githubusercontent.com/46653985/194891632-fec7fb7c-5752-4a3a-ab5d-685eb9d28d42.png)

![image](https://user-images.githubusercontent.com/46653985/194891646-22f36d9a-b984-4f33-89cb-a68d3fda4a4a.png)
...
Процесс настройки состоит из двух этапов -- автоматизированно (обязательный) и ручного (дополнительный, при необходимости тонкой настройки). Предназначение файлов конфигурации и их параметров **подробно расписано в документации** (ССЫЛКА)):
...
Основные виды диагностики, предоставляемые *Natch*, расписаны в п. 5 руководства ССЫЛКА. Следует ознакомиться со списками:
