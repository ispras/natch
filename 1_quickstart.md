...
**Решение**: на данный момент мы не делаем официальные сборки для этой системы, но использовать Natch все равно можно. Для этого оставить в папке libs библиотеки ``libSDL2-2.0.so.0`` и ``libvirglrender.so.1``, так же может потребоваться установить библиотеку ``libfdt1``. Если вы используете дистрибутив с Gaurdant, то следует еще оставить библиотеку ``libgrdlic.so``. После этого Natch будет работать без ошибок. SNatch устанавливается без дополнительных манипуляций.
