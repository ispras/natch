...
## <a name="api_config_label"></a>3.2. Modules
...
- Для подгрузки дополнительных ELF-файлов с отладочной информацией (подробнее в [ССЫЛКА](#module_config_label))
...
Конфигурационный файл со списком модулей может быть сгенерирован автоматически при помощи входящего в поставку скрипта *module_config.py* во время выполнения скрипта *natch_run.py* (подробнее в разделе [ССЫЛКА](#natch_run_label)).
...
```
## <a name="tasks_config_label"></a>3.2. Tasks
...
## <a name="syscalls_config_label"></a>3.3. Syscalls
