...
- Полe *log*: содержит название файла, в который в процессе работы будет записываться подробный лог помеченных данных (подробнее в пункте [Подробная трасса помеченных данных](5_functional.md#taint_log)).
- Полe *params_log*: содержит название файла, в который в процессе работы будет записываться лог с помеченными параметрами функций (подробнее в пункте [Получение областей помеченной памяти для функций](5_functional.md#taint_params_log)).
...
- Поле *taint*: определяет режим сбора покрытия кода (подробнее в пункте [Анализ покрытия бинарного кода](5_functional.md#functional_coverage)).
...
### 3.2.2. Автоматическая загрузка отладочной информации для разделяемых библиотек:

Конфигурационный файл может быть дополнен информацией о разделяемых бибилотеках при помощи входящего в поставку скрипта *debug_info.py*. В процессе работы, для каждого исполняемого файла происходит поиск необходимых для него разделяемых библиотек. После чего, для каждой такой библиотеки, загружается отладочная инфорамация.

Перед работой нужно установить зависимости из списка *requirements.txt*, хранящегося в папке *guest_system*, с помощью команды:
```bash
sudo pip3 install -r requirements.txt
```
И пакет, позволяющий выполнять монтирование образов:
```bash
...
- Флаг включения диагностических сообщений.
- Путь к директории, в которую будут сохранены разделяемые библиотеки и их отладочные символы. (текущая по умолчанию)

```bash
debug_info.py [-h] [-d] [-s SAVE] cfg_path img
```

В результате работы скрипта, конфигурационный файл будет дополнен информацией о разделяемых библиотеках и связанной с ними отладочной информацией. Для хранения разделяемых библиотек и отладочной информации, в диррекории, указанной в параметрах запуска, будет создана поддиректория *libs*, содержащая ещё две поддиректории: *src* и  *dbg*.

Разделяемые библиотеки, загружаемые во время выполнения с помощью dlopen и других похожих механизмов, не могут быть определены статически, поэтому зависимости, загружаемые с помощью этих механизмов, будут пропущены

В случае ошибки монтирования, убедитесь, что не смонтировано других образов или же не запущена виртуальная машина (например, Virtualbox).
...
При необходимости копирования файлов или же дирректорий из гостевой системы в хостовую, необходимо воспользоваться скриптом *copy_files.py*.
...
- Список, содержащий путеи до файлов/дирректорий (обязательный).
- Флаг включения диагностических сообщений.
...
При копировании дирректории, вложенность удаляется, т.е все файлы будут скопированы в корень указанной дирректории.
