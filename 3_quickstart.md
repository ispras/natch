...
#### <a name="map_gen"></a>3.1.1.3. Генерация map-файлов средствами компилятора
...
#### <a name="autoconfig"></a>3.2.2.1. Автоматизированная настройка
...
### <a name="automation"></a>3.2.5. Автоматизированное выполнение

Выполнение шагов 3.2.2 - 3.2.4 может быть автоматизировано средствами bash и expect. Для этих целей в архиве с тестовым примером также присутствует скрипт automation.sh, а также скрипты автозаписи сценариев для примеров (run_record_sample1.exp и run_record_sample2.exp). Необходимо запускать скрипт automation.sh. Он выполняет следующие действия:

* Проверяет наличие требующихся пакетов (expect, redis-tools), и предлагает установить их в случае отсутствия.
* Удаляет последний проект, созданный при автоматизированном выполнении.
* Создает новый проект в подкаталоге `autotest`. Создание нового проекта вручную [описано в разделе 3.2.2.1](3_quickstart.md#autoconfig).

Далее, для каждого из двух примеров
* Записывает тестовый сценарий, используя скрипт run_record_sampleX.exp. Запись сценария вручную [описана в разделе 3.2.3](3_quickstart.md#record_scenario)
* Выполняет воспроизведение записанного сценария с целью получения объекта анализа. Воспроизведение сценария детально описано в [разделе 3.2.4](3_quickstart.md#replay_scenario).

Скрипты содержат комментарии, так что, при необходимости, можно адаптировать их под свои проекты.

После выполнения в подкаталоге `autotest` появятся архивы autotest+sample1.tar.zst и autotest+sample2.tar.zst для дальнейшего анализа.


### <a name="snatch_analysis"></a>3.2.6. Анализ с использованием SNatch
...
### 3.2.8. Просмотр покрытия кода в IDA Pro
...
### 3.2.9. Изменение настроек проекта дополнительными скриптами
