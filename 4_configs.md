...
Конкретные параметры сокета, такие как ip адрес и порт назначения, можно указать через вертикальную черту. Ниже приведены примеры, указываемых имен файлов:
