...
```
Выполнение скрипта с параметром *light* приведет к тому, что в основном конфигурационном файле будет отключено построение графов, а конфигурационные файлы помеченных данных
для сценариев станут недействительны (источники пометки будут отключены).

Переключение в стандартный режим происходит аналогичным образом:
```bash
./change_settings.py -M normal
```
Конфигурационные файлы для сценариев вернутся в исходное положение.
