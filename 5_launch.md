...
Для использования инструмента следует скачать и установить *Natch* (подробнее в разделе [Комплект поставки Natch](1_quickstart.md#complect)).

Для работы *Natch* (кроме сборки для ОС Alt) потребуется установка ряда зависимостей. Сделать это можно с помощью скрипта *setup_requirements.sh*, который находится в папке *natch_scripts* (*Natch* устанавливается в директорию `/usr/bin/natch/bin/`). В результате работы будут установлены python пакеты и deb пакет, необходимые для работы скриптов, входящих в поставку инструмента. В процессе работы потребуется пароль администратора. Подробнее про установку в разделе [Установка Natch](3_setup.md#setup_natch).
...
### <a name="natch_change_settings"></a>5.2.1. Внесение изменений в скрипты запуска
