...
Для получения аргументов функций, работающих с помеченными данными, после анализа с помощью инструмента Natch, необходимо раскомментировать секцию *[FunctionArgs]* в основном конфигурационном файле Natch и указать путь к конфигурационному файлу, содержащему имена интересующих функций. Для минимизации замедления рекомендуется также закоментировать секцию, относящуюся к тейнту. Если же имена интересующих функций известны заранее, их можно указать непосредственно в конфигурационном файле в секции *[FunctionArgs]*, чтобы избежать повторного запуска.

В результате работы в дирректорию *output* будет сохранен файл *args_info.json*, содержащий результирующую информацию об аргументах в формате JSON.
