...
После проведения анализа при помощи инструмента Natch и определения функций, работающих с помеченными данными, можно получить аргументы этих функций. Для этого необходимо воспользоваться плагином function_info, который принимает два аргумента:

* *config* - указывается имя конфигурационного файла, содержащего имена интересующих функций.
* *output* - путь к директории, где будет создан файл args_info.json, содержащий результирующую информацию об аргументах в формате JSON.
...
name=test_2
```
В настоящее время, плагин способен обрабатывать только простые типы аргументов:
...
Помимо ограничения на тип аргументов, существует ещё и ограничение на поддерживаемые архитектуры. В данный момент поддерживаются только x64 и i386.

Для корректной работы данного плагина необходимо также подключить плагины  *call_stack* и *module_monitor*, а также убедиться, что в конфигурационном файле, переданном в модуль монитор, указаны нужные модули, содержащие отладочную информацию в формате DWARF. Без этого анализ не будет возможен.

Рекомендуется перед запуском плагина воспользоваться технологией записи сценария работы виртуальной машины c последующем воспроизведением его с уже подключенными плагинами. Это позволит ускорить процесс анализа.

Пример запуска плагина:

```
-plugin function_info,config=path/to/function.cfg,output=/path/to/output/folder \
-plugin module_monitor,config=path/to/module.cfg \
-plugin call_stack \
```
