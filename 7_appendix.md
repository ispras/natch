...
2. Открыть браузер и ввести строку ``localhost:3189``

3. Откроется главная страница ``Guardant Control Center``, на которой в правом верхнем углу нужно нажать на шестеренку, ввести пароль admin и попасть в раздел ``Настройки``, в нем перейти в раздел ``Поиск удаленных лицензий``.

4. Следует выключить широковещательный поиск, а в поле ``Поиск лицензий по списку адресов`` указать адрес 10.10.43.10, затем ``Применить``.

5. В верхнем меню перейти в пункт ``Ключи``, где с небольшой задержкой появится ключ с сетевой лицензией.
...
# <a name="module_config"></a>Приложение 3. Формат списка исполняемых модулей (module_config.cfg)
