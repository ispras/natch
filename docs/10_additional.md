...
Описание секции в разделе [Конфигурационный файл для помеченных данных taint.cfg](#taint_config).
...
раскомментировать секцию `Coverage` в [конфигурационном файле для помеченных данных](#taint_config).
...
В *Natch* это реализовано командой [`natch coverage extract`](#cmd_coverage).
