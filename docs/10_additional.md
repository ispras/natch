...
Переключаются режимы с помощью основного конфигурационного файла:
...
Для этого используется встроенный скрипт cov2lcov.py, который расположен в директории
`/usr/bin/natch-bin/bin/natch_scripts/guest_system/cov_convert/`.
...
Копирование исходных файлов анализируемого приложения из гостевой системы в хостовую является излишним,
поскольку необходимые файлы будут автоматически перенесены в процессе работы соответствующего скрипта.
...
В результате выполнения скрипта генерируется ряд файлов, в том числе cov.info (при использовании опции *--file*,
имя может отличаться, однако расширение .info остается неизменным). Затем можно создать отчет в формате HTML с помощью утилиты genhtml,
