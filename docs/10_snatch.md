...
# Анализ поверхности атаки с помощью SNatch
...
Установка *SNatch* описана в подразделе [Установка графической подсистемы SNatch](3_setup.md#setup_snatch).
...
в качестве данных при формировании обращений об обнаруженных багах. Помимо этого ведется логирование работы в файл `snatch.log`,
который расположен в каталоге */var/log/*.

## Регистрация пользователя и вход в *SNatch*

При первом запуске *SNatch* (либо после логаута) появляется страница входа.

<img src=images/snatch/snatch_login.png><figcaption>_Страница входа_</figcaption>

Если пользователь еще не был создан, то можно выполнить регистрацию нового пользователя (после чего автоматически будет выполнен вход).
К паролю предъявляются минимальные требования по длине (4 символа), любой сложности.
...
<img src=images/snatch/snatch_module_infoblock.png><figcaption>_Информационный блок_</figcaption>
...
Чтобы можно было увидеть исходные коды исследуемого ПО, потребуется выполнить команду [`natch coverage extract`](5_natch_cmd.md#cmd_coverage),
...
(во время воспроизведения сценария должны быть помечены порты, по которым идёт обращение с хоста к виртуальной машине). При этом в архив помещается \*.pcap файл. Для интерактивной работы с предоставленными данными необходимо иметь
...
### Управление интерактивными элементами вкладок
...
<a name="changedatadir"></a>

## Изменение каталога для хранимых проектов

Данные загруженных в SNatch проектов по умолчанию хранятся в каталоге */home/$USER/snatch/media/*. В случае недостаточного количества места в данном каталоге, каталог для хранения проектов может быть изменён. Для этого запустите скрипт */usr/bin/snatch/changedatadir.sh*. После запуска скрипта понадобится ввести путь к новой директории. Заранее убедитесь, что у вашего пользователя имеются достаточные права для создания директории или укажите существующий каталог. Для изменения настроек в SNatch потребуются права суперпользователя. Скрипт предложит удалить старый каталог или сохранить его. Также в конце будет выполнен перезапуск SNatch.

Обратите внимание, что после изменения каталога проекты, загруженные в прежний каталог перестанут работать даже при условии сохранения старого каталога. Вам необходимо будет заново загрузить соответствующие архивы в SNatch.
...
Отладочная информация выводится в файл `snatch.log` в */var/log*.
