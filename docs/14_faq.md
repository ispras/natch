...
**Проблема**: непонятно какой дистрибутив скачать

**Решение**: в связи с переходом на новую версию аппаратных ключей инструмента лицензирования Sentinel
до окончания действия всех выданных лицензий будут поддерживаться два варианта дистрибутива.
Если у вас старые ключи Sentinel, то следует брать дистрибутив из папки Sentinel_YM, если вы новый
пользователь *Natch* -- дистрибутив для вас в папке Sentinel_XE.

В постфиксах указаны первые символы кодов вендора системы лицензирования Sentinel -- YMYCK и XEKDC.
Этот код можно увидеть на обратной стороне вашего аппаратного ключа с лицензией
(либо определить по числовому коду в соответствующем поле графы ключа в Sentinel Admin Control Center
после установки драйверов Sentinel Runtime: 101213 для YMYCK и 36343 для XEKDC).

Также рекомендуется переустановить окружение (aksusbd\_*current_version*\_amd64.deb для Ubuntu/Debian/Astra),
пакет находится в папке с дистрибутивом. Для Alt необходимо выполнить `epm play aksusbd`.
...
См. [natch tuning](5_natch_cmd.md#cmd_tuning). В случае проблем с тюнингом вашей гостевой ОС, обратитесь в техническую поддержку.
...
Подробнее в разделе [Интерактивный режим создания проекта](7_create_project.md#natch_run_script).
...
**Проблемы**:
- Страница `http://localhost:8000` не загружается
- При выборе архива возникает сообщение `Task handler does not respond...` / `Обработчик задач не отвечает...` с предложением запустить `snatch_start.sh`.
- При загрузке архива в SNatch прогресс-бар не появляется, в левой панели процентное значение увеличивается до 100, после чего итоги разбора архива не загружаются или загружаются частично.

При этом установка SNatch выполнена без ошибок, `configure.sh` и `run.sh` выполнены (только для Alt Linux и РЕД ОС).

**Решение**: В этих случаях рекомендуется открыть в текстовом редакторе файл `/usr/bin/snatch/snatch_start.sh` и в строках запуска `celery` и веб-сервера `manage.py` заменить `/dev/null` на `/tmp/<logfilename>` следующим образом:
```
nohup celery -A myproduct worker -l INFO > /tmp/celery.log 2>&1 &
...
nohup python manage.py runserver --insecure 127.0.0.1:5555 > /tmp/django.log 2>&1 &
```
При этом иногда перед `celery` и `manage.py` может быть указан путь, который необходимо сохранить в команде.

Затем, находясь в том же каталоге `/usr/bin/snatch/`, выполните `run.sh` (если присутствует) или `snatch_start.sh`, после чего посмотрите созданные файлы журналов.
Обычно проблемы могут возникать при условии, если `celery` или `manage.py` не были найдены. В этом случае можно вручную указать правильный путь к ним. При корректной установке `manage.py` находится в `/usr/bin/snatch/`, а `celery` располагается в зависимости от ОС (`/home/$USER/.local/bin/` или `/opt/snatch/venv/env/bin/`).

Детали вашей проблемы просим сообщить в техническую поддержку.

-----------------------------------------------------------------------------------

**Проблема**: SNatch хранит распакованные проекты в директории пользователя, а у меня там мало места. Могу я выбрать другой каталог?

**Решение**: Откройте в текстовом редакторе файл `/usr/bin/snatch/snatch/settings.py` и отредактируйте путь в параметре `MEDIA_ROOT`. Убедитесь, что на указываемый там каталог у текущего пользователя достаточно прав. Перейдите в каталог `/usr/bin/snatch/`. Для Alt Linux и РЕД ОС запустите `run.sh`, для остальных ОС - `snatch_stop.sh`, а затем `snatch_start.sh`.
