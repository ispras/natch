...
запуск тестовых примеров для ознакомления с инструментом -- в разделе [Запуск тестовых примеров Natch](#natch_stepbystep),
подготовка образа исследуемой системы -- в разделе [Настройка окружения для работы с Natch](#setup_env),
описание команд для работы с инструментом -- в разделе [Командный интерфейс Natch](#natch_cmd),
запись и воспроизведение сценариев работы с исследуемым ПО -- в разделе [Запись и воспроизведение сценариев](#record_replay),
описание источников пометки -- в разделе [Определение источников пометки](7_taint_source.md#taint_source),
анализ и оценка результатов в *SNatch* -- в разделе [Анализ поверхности атаки с помощью SNatch](#snatch).
...
Рекомендации по подготовке объекта оценки приведены в разделе [Рекомендации по подготовке и анализу объекта оценки](#app_preparation).
