...
запуск тестовых примеров для ознакомления с инструментом -- в разделе [Запуск тестовых примеров Natch](4_launch_test_samples.md#natch_stepbystep),
подготовка образа исследуемой системы -- в разделе [Подготовка образа с объектом оценки](6_prepare_image#prepare_image),
описание команд для работы с инструментом -- в разделе [Командный интерфейс Natch](5_natch_cmd.md#natch_cmd),
запись и воспроизведение сценариев работы с исследуемым ПО -- в разделе [Запись и воспроизведение сценариев](9_scenario_work.md#record_replay),
описание источников пометки -- в разделе [Определение источников пометки](8_taint_source.md#taint_source),
анализ и оценка результатов в *SNatch* -- в разделе [Анализ поверхности атаки с помощью SNatch](10_snatch.md#snatch).

## Возможности Natch/SNatch
...
* Построение стеков вызовов для программ на Python, Java, JavaScript или C#, обрабатывающих помеченные данные
...
* Интерпретаторы и jit-компиляторы (для Python, Java, JavaScript или C#), не входящие в поставку ОС, нужно извлечь из ВМ. Интерпретаторы *обязательно* должны быть с отладочной информацией

Рекомендации по подготовке объекта оценки приведены в разделе [Рекомендации по подготовке и анализу объекта оценки](app7_oo_preparation.md#app_preparation).
