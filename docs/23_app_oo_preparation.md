...
и в гостевой и хостовой системах имелись одинаковые экземпляры (может быть сделано автоматически
...
Включается виртуализация с помощью ключа `-enable-kvm` или можно воспользоваться командой `natch kvm`.
