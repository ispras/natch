...
Подробная инструкция находится в разделе [Настройка окружения для использования лицензированного Natch](#app_license).
...
[Предыдущие релизы](#app_releases)
...
### 2.2.3. Установка зависимостей (Debian 13)

Для хостового Debian 13 требуется сервис unionfs, отсутствующий в APT-репозитории. Он необходим только в случае работы с контейнерами в гостевых системах. [По ссылке](https://pkgs.org/download/unionfs-fuse) можно скачать и установить deb пакет от Debian 12.
Все остальные зависимости на Debian 13 устанавливаются автоматически.

(setup_snatch)=

## 2.3. Установка графической подсистемы SNatch
...
Для обновления *SNatch* предварительно требуется удаление установленной версии. Для удаления *SNatch* из системы без удаления файла лога и каталога с хранимыми проектами используйте команду:
```
sudo apt-get remove snatch
```
Следует обратить внимание, что при последующей установке *SNatch* вы не сможете открыть загруженные ранее проекты. Потребуется заново загрузить соответствующие архивы в *SNatch*.
...
После первого ввода пароля суперпользователя на Alt 11 появится запрос *Введите новый пароль суперпользователя* для БД PosgreSQL. Задайте и повторите его. Через пару секунд его потребуется ввести снова.
