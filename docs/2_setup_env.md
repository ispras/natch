...
Для использования возможностей аппаратной виртуализации систему в VirtualBox следует настроить следующим образом:
...
В целом, это все. Теперь у вас есть система, поддерживаемая инструментом *Natch* и можно переходить к подготовке
образа с вашим программным продуктом.
...
Например, с случае с VirtualBox устройство `/dev/kvm` внутри Ubuntu появится только после того, как вы успешно пробросите
