...
После установки ОС, систему необходимо выключить и донастроить в интерфейсе VirtualBox как показано на
рисунках ниже. Эти настройки нужны для использования возможностей аппаратной виртуализации:
...
В целом, это все. Теперь у вас есть система, поддерживаемая инструментом, и можно
...
Важным условием выбора инструментов виртуализации и хостовой ОС является возможность запуска эмулятора QEMU в режиме аппаратной виртуализации (ключ `-enable-kvm`). Скорость работы в режиме аппаратной виртуализации более чем на порядок
превосходит работу в режиме полносистемной эмуляции.

Например, в случае с VirtualBox, устройство `/dev/kvm` внутри Ubuntu появится только после того, как вы успешно пробросите виртуализацию из вашей ОС внутрь виртуальной машины (через интерфейс VirtualBox). Если этого не сделать, ядро Ubuntu не сможет инициализировать модуль KVM, и файла `/dev/kvm` в системе не будет.
