...
Для этого в `module.cfg` предусмотрено поле `coverage`, значение которого следует установить в `True`.
Подробнее в приложении [Формат списка исполняемых модулей](18_app_module_cfg.md#app_module_config).
