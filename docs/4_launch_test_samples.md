<a name="natch_stepbystep"></a>

# Запуск тестовых примеров Natch
...
(подробности в разделе [Анализ поверхности атаки с помощью SNatch](10_snatch.md#snatch)).
...
Предполагается, что *Natch* и зависимости уже установлены в системе, но если это не так, перейдите в раздел [Установка и настройка Natch](3_setup.md#setup_natch).

<a name="test_suite"></a>

## Получение образа и тестовых примеров
...
подробнее можно ознакомиться в разделе [Автоматизированное выполнение](12_automation.md#automation).
...
<a name="config_natch_test_image"></a>

## Создание проекта для работы с тестовым образом
...
Подробно [командный интерфейс Natch](5_natch_cmd.md#natch_cmd) описан в соответствуюшем разделе.
...
Создание проекта выполняется командой `natch create` (подробнее в разделе [Создание проекта](7_create_project.md#create_project)),
...
характеристиками. Подробнее в разделе [Создание проекта с помощью конфигурационного файла](7_create_project.md#natch_save_settings).

<a name="record_scenario"></a>

## Запись сценария работы
...
<a name="replay_scenario"></a>

## Воспроизведение сценария и сбор данных для анализа

Для воспроизведения нужно выполнить команду `natch replay`.
```text
user@natch1:~/natch_quickstart/test1/$ natch replay
```
Команда может принимать ряд параметров (см. [natch replay](5_natch_cmd.md#cmd_replay)).
...
<a name="snatch_analysis"></a>

## Анализ с использованием SNatch
...
Полное руководство пользователя *SNatch* доступно в разделе [Анализ поверхности атаки с помощью SNatch](10_snatch.md#snatch), здесь же приведем краткий обзор некоторых аналитик.
...
## Просмотр сетевого трафика в Wireshark
...
Ознакомиться с подробным описанием возможностей команды `natch` можно в разделе [Командный интерфейс Natch](5_natch_cmd.md#natch_cmd).
