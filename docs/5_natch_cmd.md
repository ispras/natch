...
```text
Natch_v.3.4.1
Copyright (c) 2020-2026 ISP RAS

based on QEMU emulator v.7.2.0 (v7.2.0-5841-g0e24fa5d18-dirty)
Copyright (c) 2003-2022 Fabrice Bellard and the QEMU Project developers

Available builds: x86_64
...
Start work:
   image        create new qcow2 image
   kvm          launch QEMU kvm without Natch (only for x86_64)
   files        copy/extract files between host and guest
...
Команды инструмента разделены на логические группы, которые содержат ряд субкоманд основной команды `natch`.

Группа **Start work** предлагает пользователю набор команд для первоначальной настройки образа с объектом оценки:

- [``image``](#cmd_image)
- [``kvm``](#cmd_kvm)
- [``files``](#cmd_files)

Группа **Project commands** позволяет пользователю работать с функциями *Natch* на уровне проекта:
...
Группа **Scenarios commands** предоставляет команды для работы со сценариями проекта:
...
Группа **System parameters** содержит команды для настройки инструмента и оценки системы:
...
Архитектуру по умолчанию можно изменить в любой момент с помощью команды [`natch settings arch`](#cmd_settings_arch),
описанной ниже.
...
Команда `image` представлена в интерфейсе инструмента для удобства подготовки образа с объектом оценки.
Это просто обертка для утилиты `natch-qemu-img`, которая входит в набор бинарных файлов *Natch*.
Утилита используется для создания пустого образа для виртуальной машины QEMU, на который затем
устанавливливается операционная система и пользовательское ПО для анализа. Указанные параметры
команды напрямую передаются в утилиту `natch-qemu-img`.

Параметры:

- `qemu_img_args` -- список параметров для `natch-qemu-img`
...
Команда `files` отвечает за обмен файлами между образом и хостовой системой. Имеет субкоманды:

- [`extract`](#cmd_files_extract)
- [`copy`](#cmd_modules_copy)

Для работы команд `natch files` потребуется пароль администратора для монтирования образа.


<a name="cmd_files_extract"></a>
...
Пример:
```text
natch files extract -i Natch_testing_materials/test_image_debian.qcow2 -p /home/user/Sample1 -D bins -e
```

<a name="cmd_files_copy"></a>
...
Пример:
```text
natch files copy -i Natch_testing_materials/test_image_debian.qcow2 -p Natch_testing_materials/Sample2_bins -D /home/user/Sample2 -e
```
...
- `-o/--os_name` -- выбор гостевой ОС (Linux, FreeBSD, Win8, Win8.1, Win10)
...
Команда `modules` отвечает за работу с модулями в проекте. Имеет субкоманды:

- [`add`](#cmd_modules_add)
- [`update`](#cmd_modules_update)

Команды можно запускать только из рабочей директории проекта.
