...
На выходе будут сгенерированы скрипты для работы инструмента: *run_record.sh* и *run_replay.sh*, которые будут использованы для запуска *Natch*
в режиме записи и воспроизведения работы соответственно, а так же *run_qemu.sh* для запуска QEMU без *Natch*.
Запускать скрипты для записи и воспроизведения напрямую не требуется, но, возможно, возникнет необходисть внести в них изменения вручную.
