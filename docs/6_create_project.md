...
Для использования инструмента следует установить *Natch* и зависимости (подробнее в разделе [Установка и настройка Natch](#setup_natch)).
...
* [Командный интерфейс для работы с инструментом](#natch_cmd)
* [Конфигурационные файлы](#app_configs)
...
[Запись и воспроизведение сценариев](#record_replay)), а так же главный конфигурационный файл `natch.cfg`,
...
[Конфигурационные файлы Natch](#app_configs) или по ходу чтения документации.
...
- `-c/--config` -- путь к конфигурационному файлу, с помощью которого можно автоматически
  ...
  [Создание проекта с помощью конфигурационного файла](#natch_save_settings))
...
Кроме командных строк будет сгенерирована заготовка конфигурационного файла *Natch* (подробнее в разделе [Основной конфигурационный файл natch.cfg](#main_config)).
...
описанный в приложении [Конфигурационный файл для управления отладочной информацией debug_info.cfg](#debug_config).
