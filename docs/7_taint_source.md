...
сценария подробно в разделе [Запись сценария](#record),
или коротко в разделе [Запись сценария работы](#record_scenario)
...
Для пометки файлов в [конфигурационном файле помеченных данных](#taint_config) предназначена секция *[TaintFile]*.
...
Пометить полностью локальный трафик на данный момент можно через пометку сокетов (подробнее в разделе [Пометка сокетов](#taint_sockets)).

Для управления пометкой пакетов используется секция *[Ports]* конфигурационного файла `taint.cfg`
(подробнее в разделе [Конфигурационный файл для помеченных данных](#taint_config) секция *Ports*).
...
### 7.2.2. Более сложная фильтрация трафика
