...
[Интерактивный режим создания проекта](#natch_run_script).
...
полями формы, как описано в разделе [Более сложная фильтрация трафика](#taint_filter).
...
[конфигурационный файл для помеченных данных](#taint_config),
...
посмотреть список сохраненных снапшотов можно с помощью команды `natch info` (подробнее про [natch info](#cmd_info)),
...
Но сначала следует определить источники пометки. Подробнее об источниках в разделе [Определение источников пометки](#taint_source).
Определяются они в [конфигурационном файле для помеченных данных](#taint_config), отредактировать который можно с помощью
...
Помимо этого, команда `natch replay` имеет ряд других необязательных параметров (подробнее про [natch replay](#cmd_replay)).
