...
### 9.4.1. Сбор покрытия бинарного кода
...
### 9.4.2. Анализ покрытия бинарного кода с помощью IDA Pro
...
### 9.4.3. Конвертация покрытия из .cov64 формата в .lcov

При наличии исходных файлов анализируемого приложения, возможно выполнить преобразование информации о покрытии кода в формат Lcov. Этот процесс осуществляется посредством использования встроенного скрипта cov2lcov.py, который расположен в директории */usr/bin/natch/bin/natch_scripts/guest_system/cov_convert/*.  Для успешной конвертации требуется, чтобы собранное приложение содержало отладочную информацию либо имело отдельный файл с ней. Важно отметить, что следует избегать использования флагов компиляции Gcov, так как это может привести к искажению данных о покрытии кода.
...
- *cfg_path* -
  Путь к конфигурационному файлу для модулей (*обязательный*).
- *cov* -
  Путь к файлу, содержащему покрытие в формате .cov64 (*обязательный*).
- *img_num* -
  Номер интересующего модуля, из конфигурационного файла, указанного с помощью параметра ‘cfg_path’(обязательный)
- *workdir* -
  Рабочая директория проекта (*обязательный*)
- *-f, --file* -
  Название файла, содержащего результаты покрытия кода в формате lcov (По умолчанию: cov)
- *-l, --log* -
...
Копирование исходных файлов анализируемого приложения из гостевой системы в хостовую является излишним, поскольку необходимые файлы будут автоматически перенесены в процессе работы соответствующего скрипта.
...
В результате выполнения скрипта генерируется ряд файлов, в том числе cov.info (при использовании опции *--file*, имя может отличаться, однако расширение .info остается неизменным). Затем можно создать отчет в формате HTML с помощью утилиты genhtml, входящей в состав пакета программ lcov:

```bash
genhtml <file_name>.info -o <output_directory>
```
