...
[Интерактивный режим создания проекта](7_create_project.md#natch_run_script).

## Подготовка к записи сценария
...
полями формы, как описано в разделе [Более сложная фильтрация трафика](8_taint_source.md#taint_filter).
...
посмотреть список сохраненных снапшотов можно с помощью команды `natch info` (подробнее про [natch info](5_natch_cmd.md#cmd_info)),
...
Но сначала следует определить источники пометки. Подробнее об источниках в разделе [Определение источников пометки](8_taint_source.md#taint_source).
...
Помимо этого, команда `natch replay` имеет ряд других необязательных параметров (подробнее про [natch replay](5_natch_cmd.md#cmd_replay)).
