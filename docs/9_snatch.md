...
1. Кнопка для вызова главного меню, из которого можно создать новый сценарий, запустить ряд функций, относящихся к открытому сценарию, и открыть существующий сценарий.
...
Главное меню предоставляет доступ к созданию нового сценария, настройкам, экспорту открытого сценария в .pdf и информации о сценарии. Можно закрыть текущий сценарий, тогда будет осуществлен переход ко вкладке "список сценариев"
Также здесь отображается список существующих сценариев, открытие которых происходит по нажатию на имя, а удаление -- по нажатию на
кнопку с изображением корзины рядом с именем (для удаления всех сценариев можно нажать на изображение корзины рядом с заголовком *Сценарии*).
...
Просмотрщик поддерживает автоматическую подсветку исходников. Если переход в данную вкладку осуществлялся к функции, то номер соответствующей строки будет выделен желтым цветом.
...
5. Элементы управления графом. Первая кнопка (три горизонтальные черты с буквой *А*) раскрывает все ветви графа; кнопка с буквой *S* раскрывает только символизированные ветви. Повторное нажатие кнопки сворачивает граф. Нажатие на кнопки +/- рядом с веткой раскрывает/сворачивает данную ветку в дереве. Отдельно заметим, что при первичной загрузке содержимого дерева в качестве оптимизации происходит только частичная загрузка до определенной глубины. При этом справа от корневого узла появляется специцальная иконка (несколько отрезков в столбик), по нажатию на которую происходит полная догрузка графа.
